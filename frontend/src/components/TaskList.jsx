...
function TaskList({
  ...
  if (loading) {
    return (
      <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.04] p-12 text-center shadow-2xl backdrop-blur-xl">
  ...
  if (tasks.length === 0) {
    return (
      <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.04] p-12 text-center shadow-2xl backdrop-blur-xl sm:p-16">

        {/* Decorative Glow */}
        <div className="pointer-events-none absolute -right-24 -top-24 h-56 w-56 rounded-full bg-violet-500/10 blur-[80px]" />

        <div className="pointer-events-none absolute -bottom-24 -left-24 h-56 w-56 rounded-full bg-cyan-500/10 blur-[80px]" />

        <div className="relative flex flex-col items-center">

          <div className="mb-6 flex h-24 w-24 items-center justify-center rounded-[28px] border border-white/10 bg-[#080D1D] shadow-2xl">
  ...
  return (
    <div className="space-y-4">
      ...
      {tasks.map((task) => (
        ...
        <div
          key={task._id}
          className={`group relative overflow-hidden rounded-[26px] border p-5 shadow-xl backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl sm:p-6 ${
            task.completed
        ...
        >
          ...
          <div className="relative flex items-start gap-4">
            ...
            <button
              ...
              className="mt-1 shrink-0 rounded-full outline-none transition-all duration-200 hover:scale-110 focus:ring-4 focus:ring-violet-500/20"
            >
              {task.completed ? (
                <CheckCircle2
                  size={29}
            ...
            <div className="min-w-0 flex-1">
              ...
              <h3
                className={`break-words text-lg font-bold tracking-tight transition-all duration-200 sm:text-xl ${
                  task.completed
              ...
              {task.description && (
                <p
                  className={`mt-2 max-w-3xl break-words text-sm leading-6 ${
                    task.completed
                      ? "text-slate-600"
                      : "text-slate-400"
                  }`}
            ...
            <div className="flex shrink-0 gap-2">
              ...
              <button
                ...
                title="Edit Task"
                className="flex h-10 w-10 items-center justify-center rounded-xl border border-violet-400/10 bg-violet-500/10 text-violet-400 transition-all duration-200 hover:-translate-y-1 hover:border-violet-400/30 hover:bg-violet-500/20 hover:text-violet-300 hover:shadow-lg hover:shadow-violet-500/10 focus:outline-none focus:ring-4 focus:ring-violet-500/20"
              >
                <Edit3 size={17} />
              </button>
              ...
              <button
                ...
                title="Delete Task"
                className="flex h-10 w-10 items-center justify-center rounded-xl border border-red-400/10 bg-red-500/10 text-red-400 transition-all duration-200 hover:-translate-y-1 hover:border-red-400/30 hover:bg-red-500/20 hover:text-red-300 hover:shadow-lg hover:shadow-red-500/10 focus:outline-none focus:ring-4 focus:ring-red-500/20"
              >
                <Trash2 size={17} />
              </button>
